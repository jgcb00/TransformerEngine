...
class FusedAdemamix(torch.optim.Optimizer):
    ...
    :class:`te.optimizers.FusedAdemamix` may be used as a drop-in replacement for ``torch.optim.Ademamix``

        opt = te.optimizers.FusedAdemamix(model.parameters(), lr = ....)
        ...
        opt.step()

    Ademamix was been proposed in `The AdEMaMix Optimizer : Better, Faster, Older`_.
    ...
    Arguments:
        ...
        alpha (float, optional): coefficient used for scaling Beta3 momentum term. (default: 8.0)
        eps (float, optional): term added to the denominator to improve
            numerical stability. (default: 1e-8)
        weight_decay (float, optional): weight decay (L2 penalty) (default: 0)
        amsgrad (boolean, optional): whether to use the AMSGrad variant of this
            algorithm from the paper `On the Convergence of Ademamix and Beyond`_
            (default: False) NOT SUPPORTED in FusedAdemamix!
        adam_w_mode (boolean, optional): Apply L2 regularization or weight decay
            True for decoupled weight decay(also known as AdamW) (default: True) (recommended : True)
    ...
    def step(self, closure=None, grad_scaler=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(state) == 0:
                    # Slow exponential moving average of gradient values
                    state["exp_avg_slow"] = torch.zeros_like(p.data).float()
                    # Fast exponential moving average of gradient values
                    state["exp_avg_fast"] = torch.zeros_like(p.data).float()
                ...
                if isinstance(p, Float8Tensor):
                    ...
                    m1_of_fp8_model.append(state["exp_avg_fast"])
                    m2_of_fp8_model.append(state["exp_avg_slow"])
                    ...
                    m1_of_f16_model.append(state["exp_avg_fast"])
                    m2_of_f16_model.append(state["exp_avg_slow"])
                    v_of_f16_model.append(state["exp_avg_sq"])
                elif p.dtype == torch.float32:
                    p_f32_model.append(p.data)
                    g_of_f32_model.append(p_grad.data)
                    m1_of_f32_model.append(state["exp_avg_fast"])
                    m2_of_f32_model.append(state["exp_avg_slow"])
                    v_of_f32_model.append(state["exp_avg_sq"])
                else:
                    raise RuntimeError("FusedAdemamixemix only support model weights in fp16/bf16 and fp8")

                if self.capturable and len(p_fp8_model) > 0:
                    raise RuntimeError(
                        "FusedAdemamixemix does not support FP8 model weights with capturable=True."
                    )

                if has_fp16 and has_bf16:
                    # simple to add support for this, but not needed for now
                    raise RuntimeError(
                        "FusedAdemamixemix does not support a mix of float16 and bfloat16 model weights."
                    )
            ...
            if self.capturable:
                ...
                if self.master_weights:
                    ...
                    if len(p_f16_model) > 0:
                        tensor_lists = [g_of_f16_model, p_f16_model, m1_of_f16_model, m2_of_f16_model, v_of_f16_model]
                        apply_multi_tensor_ademamix(
                            self.multi_tensor_ademamix_capturable, tensor_lists, inv_scale
                        )
                    if len(p_f32_model) > 0:
                        tensor_lists = [g_of_f32_model, p_f32_model, m1_of_f32_model, m2_of_f16_model, v_of_f32_model]
                        apply_multi_tensor_ademamix(
                            self.multi_tensor_ademamix_capturable, tensor_lists, inv_scale
                        )
                ...
                if len(p_f16_model) > 0:
                    ...
                    apply_multi_tensor_ademamix(self.multi_tensor_ademamix, tensor_lists)
                if len(p_f32_model) > 0:
                    tensor_lists = [g_of_f32_model, p_f32_model, m1_of_f32_model, m2_of_f32_model, v_of_f32_model]
                    apply_multi_tensor_ademamix(self.multi_tensor_ademamix, tensor_lists)
