...
struct SpamFunctorMaster {
  ...
  __device__ __forceinline__ void operator()(index_t chunk_size, volatile int *noop_gmem,
                                             ...
                                             const float lr, spamMode_t mode, const float decay) {
...
struct spamCapturableFunctor {
  __device__ __forceinline__ void operator()(int chunk_size, volatile int *noop_gmem,
                                             TensorListMetadata<4> &tl,  // NOLINT(*)
                                             const float beta1, const float beta2, const float theta, const int *step,
...
template <typename T, typename FULL_T>
struct spamCapturableMasterFunctor {
  __device__ __forceinline__ void operator()(int chunk_size, volatile int *noop_gmem,
...
void multi_tensor_spam_cuda(int chunk_size, at::Tensor noop_flag,
                            std::vector<std::vector<at::Tensor>> tensor_lists, const float lr,
                            const float beta1, const float beta2, const float theta, const float epsilon,
...
void multi_tensor_spam_fp8_cuda(int chunk_size, at::Tensor noop_flag,
                                std::vector<std::vector<at::Tensor>> tensor_lists, const float lr,
                                const float beta1, const float beta2, const float theta, float epsilon,
                                const int step, const int mode, const int bias_correction,
...
void multi_tensor_spam_capturable_cuda(int chunk_size, at::Tensor noop_flag,
                                       std::vector<std::vector<at::Tensor>> tensor_lists,
                                       at::Tensor lr, const float beta1, const float beta2, const float theta,
  ...
  DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
      tensor_lists[0][0].scalar_type(), 0, "spam",
      multi_tensor_apply<4>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                            spamCapturableFunctor<scalar_t_0, float>(), beta1, beta2, theta,
                            step.data_ptr<int>(), bias_correction, epsilon, lr.data_ptr<float>(),
                            (spamMode_t)mode, weight_decay, inv_scale.data_ptr<float>());)

  AT_CUDA_CHECK(cudaGetLastError());
}

void multi_tensor_spam_capturable_master_cuda(int chunk_size, at::Tensor noop_flag,
                                              std::vector<std::vector<at::Tensor>> tensor_lists,
                                              at::Tensor lr, const float beta1, const float beta2, const float theta,
                                              const float epsilon, at::Tensor step, const int mode,
                                              const int bias_correction, const float weight_decay,
  ...
  DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
      tensor_lists[0][0].scalar_type(), 0, "spam",
      multi_tensor_apply<5>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                            spamCapturableMasterFunctor<scalar_t_0, float>(), beta1, beta2, theta,
                            step.data_ptr<int>(), bias_correction, epsilon, lr.data_ptr<float>(),
                            (spamMode_t)mode, weight_decay, inv_scale.data_ptr<float>());)
