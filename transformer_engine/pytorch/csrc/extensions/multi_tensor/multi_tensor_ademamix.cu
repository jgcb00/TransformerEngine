...
struct AdemamixFunctorMaster {
  ...
  __device__ __forceinline__ void operator()(index_t chunk_size, volatile int *noop_gmem,
    ...
    FULL_T *m1 = reinterpret_cast<FULL_T *>(tl.addresses[2][tensor_loc]);
    m1 += chunk_idx * chunk_size;

    FULL_T *m2 = reinterpret_cast<FULL_T *>(tl.addresses[3][tensor_loc]);
    m2 += chunk_idx * chunk_size;
...
#pragma unroll
      for (int ii = 0; ii < ILP; ii++) {
        ...
        if (i < n && i < chunk_size) {
          ...
          r_m1[ii] = static_cast<MATH_T>(m1[i]);
          r_m2[ii] = static_cast<MATH_T>(m2[i]);
...
#pragma unroll
      for (int ii = 0; ii < ILP; ii++) {
        if (mode == ADAM_MODE_0) {  // L2
          ...
          MATH_T update = (next_m1_unbiased + alpha * next_m2_unbiased) / denom;
          ...
          MATH_T next_v_unbiased = r_v[ii] / beta2_correction;
          MATH_T denom = sqrtf(next_v_unbiased) + epsilon;
          MATH_T update = ((next_m1_unbiased + alpha * next_m2_unbiased) / denom) + (decay * r_p[ii]);
          r_p[ii] = r_p[ii] - (lr * update);
        }
      }
...
#pragma unroll
      for (int ii = 0; ii < ILP; ii++) {
        ...
        if (i < n && i < chunk_size) {
          ...
          m1[i] = static_cast<FULL_T>(r_m1[ii]);
          m2[i] = static_cast<FULL_T>(r_m2[ii]);
...
struct AdemamixCapturableFunctor {
  __device__ __forceinline__ void operator()(int chunk_size, volatile int *noop_gmem,
                                             TensorListMetadata<5> &tl,  // NOLINT(*)
                                             const float beta1, const float beta2, const float beta3, const float alpha, int *step,
                                             const int bias_correction, const float epsilon,
                                             const float *lr, adamMode_t mode, const float decay,
                                             const float *inv_scale) {
    if (*noop_gmem == 1) return;

    float beta1_correction = 1.0f, beta2_correction = 1.0f, beta3_correction = 1.0f;
    if (bias_correction == 1) {
      beta1_correction = 1 - pow(beta1, *step);
      beta2_correction = 1 - pow(beta2, *step);
      beta3_correction = 1 - pow(beta3, *step);
    }
...
struct AdemamixCapturableMasterFunctor {
  __device__ __forceinline__ void operator()(int chunk_size, volatile int *noop_gmem,
                                             ...
                                             const float beta1, const float beta2, const float beta3, const float alpha, const int *step,
                                             const int bias_correction, const float epsilon,
                                             const float *lr, adamMode_t mode, const float decay,
                                             const float *inv_scale) {
    if (*noop_gmem == 1) return;
...
void multi_tensor_ademamix_cuda(int chunk_size, at::Tensor noop_flag,
                            ...
                            const float beta1, const float beta2, const float beta3, const float alpha, float epsilon,
                            const int step, const int mode, const int bias_correction,
                            const float weight_decay) {
  using namespace at;

  // Handle bias correction mode
  float bias_correction1 = 1.0f, bias_correction2 = 1.0f, bias_correction3 = 1.0f;
  if (bias_correction == 1) {
    bias_correction1 = 1 - std::pow(beta1, step);
    bias_correction2 = 1 - std::pow(beta2, step);
    bias_correction3 = 1 - std::pow(beta3, step);
  }
  ...
  // case 5:  g, p, m1, m2, v
  // case 6:  g, p, m1, m2, v, p_master
  TORCH_CHECK(tl_size == 5 || tl_size == 6, "tensor list must contain 5 or 6");

  if (requires_64bit_indexing) {
    if (tl_size == 5) {
      // Assume single type across p,g,m1,m2 now
      DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
          p_in_type, 0, "ademamix",
          multi_tensor_apply<5>((int64_t)BLOCK_SIZE, (int64_t)chunk_size, noop_flag, tensor_lists,
                                AdemamixFunctor<scalar_t_0, float, int64_t>(), beta1, beta2, beta3, alpha,
                                bias_correction1, bias_correction2, bias_correction3, epsilon, lr, (adamMode_t)mode,
                                weight_decay);)
    } else {
      // g, p, m1, m2, v, p_master
      const auto g_in_type = tensor_lists[0][0].scalar_type();
      DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
          p_in_type, 0, "ademamix",
          DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
              g_in_type, 1, "ademamix",
              multi_tensor_apply<6>((int64_t)BLOCK_SIZE, (int64_t)chunk_size, noop_flag,
                                    tensor_lists,
                                    AdemamixFunctorMaster<scalar_t_0, scalar_t_1, float, int64_t>(),
                                    beta1, beta2, beta3, alpha, bias_correction1, bias_correction2, bias_correction3, epsilon, lr,
    ...
    if (tl_size == 5) {
      ...
      DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
          ...
          multi_tensor_apply<5>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                                AdemamixFunctor<scalar_t_0, float, int32_t>(), beta1, beta2, beta3, alpha,
                                bias_correction1, bias_correction2, bias_correction3, epsilon, lr, (adamMode_t)mode,
                                weight_decay);)
    } else {
      const auto g_in_type = tensor_lists[0][0].scalar_type();
      DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
          ...
          DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
              ...
              multi_tensor_apply<6>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                                    AdemamixFunctorMaster<scalar_t_0, scalar_t_1, float, int32_t>(),
                                    beta1, beta2, beta3, alpha, bias_correction1, bias_correction2,  bias_correction3, epsilon, lr,
                                    (adamMode_t)mode, weight_decay);));
    }
  }
...
void multi_tensor_ademamix_fp8_cuda(int chunk_size, at::Tensor noop_flag,
  ...
  // case 8:  g, p_fp8, m1, m2, v, p_master, scale, amax, scale_inv
  TORCH_CHECK(tl_size == 9, "tensor list must contain 9 tensors");

  if (requires_64bit_indexing) {
    TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(
        fp8_dtype, FP8_T,
        DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
            g_in_type, 0, "ademamix",
            multi_tensor_apply<6, true>(
                (int64_t)BLOCK_SIZE, (int64_t)chunk_size, noop_flag, tensor_lists,
                AdemamixFunctorMaster<FP8_T, scalar_t_0, float, int64_t>(), beta1, beta2, beta3, alpha,
                bias_correction1, bias_correction2, bias_correction3, epsilon, lr, (adamMode_t)mode, weight_decay);));
  } else {
    TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(
        fp8_dtype, FP8_T,
        DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
            ...
            multi_tensor_apply<6, true>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                                        AdemamixFunctorMaster<FP8_T, scalar_t_0, float, int32_t>(),
                                        beta1, beta2, beta3, alpha, bias_correction1, bias_correction2, bias_correction3, epsilon,
...
void multi_tensor_ademamix_capturable_cuda(int chunk_size, at::Tensor noop_flag,
                                       ...
                                       at::Tensor lr, const float beta1, const float beta2, const float beta3, const float alpha,
  ...
  DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
      tensor_lists[0][0].scalar_type(), 0, "ademamix",
      multi_tensor_apply<5>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                            AdemamixCapturableFunctor<scalar_t_0, float>(), beta1, beta2, beta3, alpha,
...
void multi_tensor_ademamix_capturable_master_cuda(int chunk_size, at::Tensor noop_flag,
  ...
  DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
      tensor_lists[0][0].scalar_type(), 0, "ademamix",
      multi_tensor_apply<6>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                            AdemamixCapturableMasterFunctor<scalar_t_0, float>(), beta1, beta2, beta3, alpha,
                            step.data_ptr<int>(), bias_correction, epsilon, lr.data_ptr<float>(),
                            (adamMode_t)mode, weight_decay, inv_scale.data_ptr<float>());)
